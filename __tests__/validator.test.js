...
describe('Validator for name query on "/name" endpoint', () => {
  let req = {
    query: {
      name: 'Lewis',
    },
  ...
  it('responds with 200 code with query "/name?name=Lewis"', async () => {
    const response = await request.get('/name?name=Lewis');
    expect(response.status).toBe(200);
  });

  it('responds with 500 code with query "/name?name="', async () => {
